...
- **ライブラリはユーザー単位で全プロジェクト共通**。歌詞や固有名詞を含めない汎用背景(16ムード)なので**他の曲でも再利用でき、使うほど蓄積**されます。保存先は `data/bglib/`、配信は `/media/bglib/<id>.png`。
...
- **背景MVスタジオ (静止画背景)**: 曲の展開(Intro/Verse/Chorus…)ごとに静止画背景を割り当て、境界でクロスフェード。各セクションで背景がゆっくりズーム/パン(Ken Burns)し、暗幕+パレット・ティントで歌詞可読性と統一感を確保。背景は **GPT Image 2 (AtlasCloud経由)** で生成し、**ユーザー単位のライブラリに蓄積**。歌詞や固有名詞を含めない16ムード(ネオン都市/夜空/夕焼け/海/森/桜/雪/雨窓/抽象流体/オーロラ/サイバー/都会ボケ/ステージ/宇宙/ミニマル/山霧)の“汎用背景”なので**他の曲でも再利用でき、使うほどバリエーションが増える**。エネルギーに応じた自動割り当ても可能。詳細は下記
