...
| 仕様 | 実装 |
|---|---|
| 2.1 認証 | メール/パスワード + JWT (access 1h / refresh 30d, 自動リフレッシュ)。RBAC (Owner/Admin/Editor/Viewer)。**MFA (TOTP, RFC 6238) — stdlib実装、登録/有効化/ログイン検証** |
...
| 2.7 B2B | テンプレートマーケットプレイス、**プロジェクトからのテンプレート出品**、ブランドキット、ホワイトラベルAPI (X-API-Key, **プラン別レート制限**) |
