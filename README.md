...
### 背景MVスタジオ (静止画背景の生成・蓄積・割り当て)

曲の展開ごとに静止画背景を敷いてMV的な雰囲気を出す機能。背景は **GPT Image 2** を **AtlasCloud** 経由で生成します。

```bash
# lyricflow-ai/.env に追記 (gitignore対象)
ATLASCLOUD_API_KEY=sk-...            # https://atlascloud.ai のAPIキー
# ATLASCLOUD_IMAGE_MODEL=openai/gpt-image-2/text-to-image   # 既定でOK
```

- エディター右の AIツール **🎬 背景MVスタジオ** から: ムードを選んで枚数指定→生成、ライブラリから各セクションへクリック割り当て、エネルギーに応じた **自動割り当て**。
- **ライブラリはユーザー単位で全プロジェクト共通**。歌詞や固有名詞を含めない汎用背景(43ムード×6カテゴリ)なので**他の曲でも再利用でき、使うほど蓄積**されます。保存先は `data/bglib/`、配信は `/media/bglib/<id>.png`。
- 濫用対策として **1ユーザー1日80枚**の生成上限 (`LYRICFLOW_BG_DAILY_LIMIT`)。キー未設定でも既存ライブラリの割り当ては可能で、割り当てが無いセクションはプロシージャル背景にフォールバックします。
- API: `GET /api/v1/bglib`(一覧+ムード+日次) / `POST /api/v1/bglib/generate {moods?,count,orient}` / `GET /api/v1/bglib/jobs`(生成状況) / `DELETE /api/v1/bglib/{id}`。
- レンダリング(`static/js/fx.js`)はセクションに割り当てられた画像を cover 配置し、Ken Burnsズーム/パン + 境界クロスフェード + 暗幕/ティストを合成。決定論エクスポートは開始前に全背景画像をプリロードします。
...
- **文字位置の自由配置(リアルタイム)**: プレビュー上で歌詞を直接ドラッグして任意の位置へ移動(掴んだ点基準)。歌詞スタイルの横位置/縦位置スライダー・中央リセットにも対応。`lyricStyle.posX/posY`(0..1)で保持し、横書き/縦書き両対応・書き出しにも反映
- **キネティック・タイポグラフィ**: **16種のレタリング** (ネオン/アウトライン/マーカー/筆/クローム/ロングシャドウ/ゴールド/2色グラデ/レインボー/ファイア/アイス/3D押し出し/グリッチ(RGBずれ)/レトロ80s/ステッカー/ピル)、**18種の出現アニメ** (グロウポップ/フェード/スライド/ポップ/グリッチ/ズーム/ドロップ弾み/ふわり上昇/スピン/フリップ/スイング/ブラー/ストレッチ/タイプライター/カスケード/ウェーブ/タンブル ほか。一字ずつ入場する系も対応)、**縦書き(tategaki)対応**、字間調整、多層グロー + easeOutBack + 発声脈動、**文字サイズのランダム化(各単語の先頭文字は必ず大きく表示)**。**出現アニメは曲全体でも、Intro/Verse/Chorus などシーンごとにも変更可(シーン解析AI後)**
- **前景キャラ/被写体レイヤー**: アニメキャラ画像/透過PNGを前景配置し、リムライト・浮遊・呼吸・接地影で演出
- **背景MVスタジオ (静止画背景)**: 曲の展開(Intro/Verse/Chorus…)ごとに静止画背景を割り当て、境界でクロスフェード。各セクションで背景がゆっくりズーム/パン(Ken Burns)し、暗幕+パレット・ティントで歌詞可読性と統一感を確保。背景は **GPT Image 2 (AtlasCloud経由)** で生成し、**ユーザー単位のライブラリに蓄積**。歌詞や固有名詞を含めない**43ムード×6カテゴリ**(都市/自然/空・宇宙/季節・和/室内・演出/抽象)の“汎用背景”なので**他の曲でも再利用でき、使うほどバリエーションが増える**。エネルギーに応じた自動割り当ても可能。詳細は下記
- **カメラドリフト (Ken Burns)**: 背景のゆっくりズーム/パン + サビでビート連動
- **カラーグレード + フィルムグレイン**: soft-light の映画的トーン + overlay グレイン
- **画面全体エフェクト(19種)**: 既存の Bloom/Glitch/色収差/Wave歪み/光芒/レンズフレア/DoF に加え、**フラッシュ(明滅)・放射ズームブラー・RGBずれ・走査線(CRT)・VHS・モザイク/ピクセル・網点(ハーフトーン)・ミラー/万華鏡(左右/上下/4分割)・色相サイクル・光漏れ・古いフィルム(セピア+傷+フリッカー)・シネスコ黒帯** を追加。各 `fx.<key>` を0〜1で強度指定、サビで自動ブースト。歌詞スタイル下の「画面全体エフェクト」から調整
